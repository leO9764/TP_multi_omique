...
pls_rna_prot_mirna = block.pls(X = blocks, Y = as.matrix(as.data.frame(liste_mirna_filtre)), ncomp = 2)
pls_rna_prot_mirna
```
...
RNA_prot_miRNA.spls = block.spls(X = blocks, Y = as.matrix(as.data.frame(liste_mirna_filtre)), ncomp = 1, 
                    mode = "regression", scale = TRUE, keepX = list(10,9), keepY = list(7))
RNA_prot_miRNA.spls
...
RNA_prot_miRNA.splsda2 = block.splsda(X = data2, Y = sample$Y, ncomp = 2, keepX = list(1,2))
RNA_prot_miRNA.splsda2
...
circosPlot(RNA_prot_miRNA.splsda2, title = "titre")
