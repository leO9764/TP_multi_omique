...
liste_mirna = apply(mirna, 2, coef_var) ; typeof(liste_mirna)
...
```{r}
liste_mirna_filtre = which(liste_mirna >= 0.15) ; typeof(liste_mirna_filtre)
liste_mrna_filtre = which(liste_mrna >= 0.15)
liste_prot_filtre = which(liste_prot >= 0.15)
```
...
max_liste_mirna = liste_mirna[181] ; max_liste_mirna # hsa.mir.96 max val = 0.2414325

max(liste_mrna_filtre)
max_liste_mrna = liste_mrna[181] ; max_liste_mrna # hsa.mir.96 max val = 0.2414325

max(liste_prot_filtre)
max_liste_prot = liste_prot[181] ; max_liste_prot # hsa.mir.96 pas dans le jeu de donnee.
```
 ...
 * Quel est le rôle de ce gène ? 
   Il diminue l'expression du récepteur hepatic SRB1, qui a pour rôle l'a prise'absorption de HDLC dans les cellules hépatiques
